...
void MarchingCubes::draw_fluid(Vector3 v[8], float d[8], SurfaceTool *st) {
	int tag = 0x00000000;
	int idx[] = {0, 1, 4, 5, 2, 3, 6, 7};
	...
	for (int i=0; i < tri_count; i++) {
		Vector3 v1, v2, v3;
		v1 = find_simple_vert(regularVertexData[tag][cell.vertexIndex[i*3]],	 v, d);
		v2 = find_simple_vert(regularVertexData[tag][cell.vertexIndex[i*3 + 1]], v, d);
		v3 = find_simple_vert(regularVertexData[tag][cell.vertexIndex[i*3 + 2]], v, d);
...
Vector3 MarchingCubes::find_simple_vert(int edge_index, Vector3 v[8], float d[8]) {
	float d1, d2;
	int idx1, idx2;
	Vector3 v1, v2;
	...
	return v1.linear_interpolate(v2, t);
}